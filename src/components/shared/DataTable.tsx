...
export function DataTable<T extends Record<string, any>>({
  ...
  const handleSort = useCallback((key: keyof T) => {
    if (sortKey === key) {
      ...
      if (sortDir === "desc") {setSortDir(null);setSortKey(null);} else
      {setSortDir("asc");setSortKey(key);}
    } else {
      setSortKey(key);
      setSortDir("asc");
    }
          ...
          <label key={v} className="flex items-center gap-2 text-sm cursor-pointer">
              ...
              onCheckedChange={(checked) => {
                setColumnEnumFilters((prev) => {
                  ...
                  if (checked) next.add(v);else
                  next.delete(v);
                  const updated = { ...prev };
                  if (next.size === 0) delete updated[key];else
                  updated[key] = next;
                  return updated;
                });
                setPage(0);
            ...
            <DropdownMenuCheckboxItem
              ...
              onCheckedChange={(checked) => {
                setVisibleColumns((prev) => {
                  ...
                  if (checked) next.add(colId);else
                  next.delete(colId);
                  return next;
                });
              }}>
