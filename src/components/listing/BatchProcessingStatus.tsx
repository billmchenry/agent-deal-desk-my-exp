...
interface BatchProcessingStatusProps {
  ...
  currentIndex: number;
  onComplete: (documents: ClassifiedDocument[]) => void;
  onUpdateDocuments?: (documents: ClassifiedDocument[]) => void;
}
...
export function BatchProcessingStatus({ 
  ...
  const getStatusIcon = (status: ClassifiedDocument['status']) => {
    switch (status) {
      case 'complete':
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <p className="text-sm text-foreground">
            {isAllComplete ? (
              <>
                <span className="font-medium">All done!</span> I've processed {completedCount} document{completedCount !== 1 ? 's' : ''}.
                {errorCount > 0 && ` (${errorCount} had issues)`}
              </>
            ) : currentDoc ? (
              <>
                Processing <span className="font-medium">{currentDoc.classification.displayName}</span>...
